 ...
 *
 */
class MoPlanDetail extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      parentIds: Router.router.query.parentIds.replace(/-/g, ',')
    });
    Dingtalk.authDingJsApi("", this.props.urlData);
    Dingtalk.setTitle("重复规则");
  }
  ...
  changeRadio = type => {
    // this.setState({
    //   orangalExecutionTimeString: this.state.nextExecutionTimeString
    // });
    if (type == "1") {
      this.setState({
        monthCount: 1,
  ...
  saveData() {
    ...
    parentIds && parentIds.split(",").map((item, index) => {
      if (index > 0) {
        parentId = item
      }
    })
    let taskInfo = {
      taskId: this.props.repeatPlanObj.antTaskinfo.id,
      taskname: this.props.repeatPlanObj.antTaskinfo.taskname,
      description: this.props.repeatPlanObj.antTaskinfo.description,
      userResponse: this.props.repeatPlanObj.antTaskinfo.userResponse,
      planEndTimeString: this.props.repeatPlanObj.antTaskinfo.planEndTimeString,
      userFlow: this.props.repeatPlanObj.antTaskinfo.userFlow,
      flowConten: this.props.repeatPlanObj.antTaskinfo.flowConten,
      coefficienttype: this.props.repeatPlanObj.antTaskinfo.coefficienttype,
      workTime: this.props.repeatPlanObj.antTaskinfo.workTime,
      labels: this.props.repeatPlanObj.antTaskinfo.labels,
      mobileFile: this.props.repeatPlanObj.antTaskinfo.mobileFile,
      category: this.props.repeatPlanObj.antTaskinfo.category,
      parentIds: parentIds,
      parent: {
        id: parentId
      }
    ...
    this.props.createRepeatTask(projectId, taskNew, (data, iserr) => {
      if (iserr) {
        Toast.fail(data);
      } else {
        Router.replace(
          Dingtalk.setDdNavColor(
            "/mo_taskDetails?id=" +
            this.props.repeatPlanObj.antTaskinfo.id +
            "&planed=1"
          )
        );
      }
            ...
            {nextExecutionTimeString && (
              <DatePicker
                ...
                onChange={date => {
                  this.setState(
                    {
                      nextExecutionTimeString: moment(date),
